            ...
            <div class="card m-b-30">
                <div class="card-body">
                    <h4 class="mt-0 header-title">
                        @if((!is_null(\App\Model\EnterpriseLimit::where('user', Auth::id())->first()) ? \App\Model\EnterpriseLimit::where('user', Auth::id())->first()->amount : __("fields.enterprise_limit_single")) > \App\Model\Enterprise::where('user', Auth::id())->count())
                            <a href="{{ url('enterprise/create') }}"
                               class="btn btn-sm btn-success">@lang('fields.btn_register_new')</a>
                        @else
                            <a href="#"
                               class="btn btn-sm btn-dark disabled">@lang('fields.btn_register_new')</a>
                        @endif
...
@section('script')
    <script>
        ...
        function allEnterprises() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('server/enterprise_all_single_enterprise') }}",
                headers: {
                        ...
                        if (data.status == "OK") {
                            swal({
                                ...
                                if (data.null != null) {
                                    ...
                                    location.href = '{{ url('enterprise/create') }}';
                                }
                            });
                        }
                    }, error: function (data) {
