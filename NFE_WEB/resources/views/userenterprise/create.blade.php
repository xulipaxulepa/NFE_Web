...
@section('title')- @lang('fields.user_title_register') @endsection

@section('content')
    <div class="row">
        <div class="col-lg-6">
            <div class="card m-b-30">
                {{ Form::open(['url' => 'userenterprise', 'method' => 'POST', 'id' => 'registerForm']) }}
                <div class="card-body">
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ url('userenterprise') }}" class="btn btn-secondary waves-effect waves-light pull-right">
    ...
    <script>
        ...
        jQuery.validator.addMethod("uniqueEmail", function (value, element, config) {
            ...
            $.ajax({
                ...
                async: false,
                data: {email: value, role: 'ROLE_MANAGER'},
            ...
            return status;
        }, "{{ __("validation_jquery.value_not_allowed") }}");

        jQuery.validator.addMethod("uniqueEmailEnterprise", function (value, element, config) {
            var status = false;
            $.ajax({
                type: "POST",
                url: "{{ url('api/user_enterprise_open_email') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                async: false,
                data: {email: value, enterprise: '{{ Session::get('enterprise')->id }}'},
                success: function (data) {
                    if (data.data == null) {
                        status = true;
                    }
